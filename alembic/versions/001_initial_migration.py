"""Initial migration for StudyGuru Pro

Revision ID: 001
Revises: 
Create Date: 2024-01-01 00:00:00.000000

"""
from alembic import op
import sqlalchemy as sa
from sqlalchemy.dialects import postgresql

# revision identifiers, used by Alembic.
revision = '001'
down_revision = None
branch_labels = None
depends_on = None


def upgrade() -> None:
    # Create enum types
    subscription_plan_enum = sa.Enum('FREE', 'BASIC', 'PRO', name='subscriptionplan')
    account_provider_enum = sa.Enum('EMAIL', 'GOOGLE', name='accountprovider')
    user_account_type_enum = sa.Enum('SUPER_ADMIN', 'ADMIN', 'USER', name='useraccounttype')
    
    subscription_plan_enum.create(op.get_bind())
    account_provider_enum.create(op.get_bind())
    user_account_type_enum.create(op.get_bind())
    
    # Create subscriptions table
    op.create_table('subscriptions',
        sa.Column('id', sa.String(191), nullable=False),
        sa.Column('subscription_name', sa.String(191), nullable=False),
        sa.Column('usd_amount', sa.Float(), nullable=False),
        sa.Column('subscription_plan', subscription_plan_enum, nullable=False),
        sa.Column('points_per_month', sa.Integer(), nullable=False, server_default='0'),
        sa.Column('is_addon', sa.Boolean(), nullable=False, server_default='false'),
        sa.Column('min_points', sa.Integer(), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.PrimaryKeyConstraint('id')
    )
    
    # Create purchased_subscription table
    op.create_table('purchased_subscription',
        sa.Column('id', sa.String(191), nullable=False),
        sa.Column('subscription_id', sa.String(191), nullable=False),
        sa.Column('paddle_subscription_id', sa.String(191), nullable=True),
        sa.Column('subscription_status', sa.String(191), nullable=True),
        sa.Column('next_billing_date', sa.DateTime(), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['subscription_id'], ['subscriptions.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    
    # Create user table
    op.create_table('user',
        sa.Column('id', sa.String(191), nullable=False),
        sa.Column('first_name', sa.String(191), nullable=False),
        sa.Column('last_name', sa.String(191), nullable=False),
        sa.Column('password', sa.String(191), nullable=True),
        sa.Column('email', sa.String(191), nullable=False),
        sa.Column('account_provider', account_provider_enum, nullable=True),
        sa.Column('account_type', user_account_type_enum, nullable=False, server_default='USER'),
        sa.Column('photo_url', sa.String(191), nullable=True),
        sa.Column('birthday', sa.DateTime(), nullable=True),
        sa.Column('country_name', sa.Text(), nullable=True),
        sa.Column('profession_title', sa.Text(), nullable=True),
        sa.Column('primary_city', sa.Text(), nullable=True),
        sa.Column('primary_state', sa.Text(), nullable=True),
        sa.Column('primary_address', sa.Text(), nullable=True),
        sa.Column('about_description', sa.Text(), nullable=True),
        sa.Column('phone_number', sa.Text(), nullable=True),
        sa.Column('zip_code', sa.Text(), nullable=True),
        sa.Column('reset_password_pin', sa.Integer(), nullable=True),
        sa.Column('reset_password_expire_date', sa.DateTime(), nullable=True),
        sa.Column('verify_pin', sa.Integer(), nullable=True),
        sa.Column('verify_pin_expire_date', sa.DateTime(), nullable=True),
        sa.Column('verify_status', sa.Boolean(), nullable=False, server_default='false'),
        sa.Column('current_points', sa.Integer(), nullable=False, server_default='30'),
        sa.Column('total_points_earned', sa.Integer(), nullable=False, server_default='30'),
        sa.Column('total_points_used', sa.Integer(), nullable=False, server_default='0'),
        sa.Column('purchased_subscription_id', sa.String(191), nullable=True),
        sa.Column('super_admin', sa.Boolean(), nullable=True, server_default='false'),
        sa.Column('last_login_at', sa.DateTime(), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['purchased_subscription_id'], ['purchased_subscription.id'], ),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('email')
    )
    
    # Create media table
    op.create_table('media',
        sa.Column('id', sa.String(191), nullable=False),
        sa.Column('original_filename', sa.String(500), nullable=False),
        sa.Column('s3_key', sa.String(500), nullable=False),
        sa.Column('file_type', sa.String(100), nullable=False),
        sa.Column('original_size', sa.Float(), nullable=False),
        sa.Column('compressed_size', sa.Float(), nullable=True),
        sa.Column('compression_ratio', sa.Float(), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.PrimaryKeyConstraint('id')
    )
    
    # Create doc_material table
    op.create_table('doc_material',
        sa.Column('id', sa.String(191), nullable=False),
        sa.Column('user_id', sa.String(191), nullable=False),
        sa.Column('file_id', sa.String(191), nullable=False),
        sa.Column('analysis_response', sa.JSON(), nullable=True),
        sa.Column('question_type', sa.String(50), nullable=True),
        sa.Column('detected_language', sa.String(50), nullable=True),
        sa.Column('title', sa.String(500), nullable=True),
        sa.Column('summary_title', sa.String(500), nullable=True),
        sa.Column('tokens_used', sa.Integer(), nullable=False, server_default='0'),
        sa.Column('points_cost', sa.Integer(), nullable=False, server_default='0'),
        sa.Column('status', sa.String(50), nullable=False, server_default='processing'),
        sa.Column('error_message', sa.Text(), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['file_id'], ['media.id'], ),
        sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    
    # Create point_transaction table
    op.create_table('point_transaction',
        sa.Column('id', sa.String(191), nullable=False),
        sa.Column('user_id', sa.String(191), nullable=False),
        sa.Column('transaction_type', sa.String(50), nullable=False),
        sa.Column('points', sa.Integer(), nullable=False),
        sa.Column('description', sa.Text(), nullable=True),
        sa.Column('doc_material_id', sa.String(191), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['doc_material_id'], ['doc_material.id'], ),
        sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    
    # Create billing_log table
    op.create_table('billing_log',
        sa.Column('id', sa.String(191), nullable=False),
        sa.Column('user_id', sa.String(191), nullable=False),
        sa.Column('subscription_id', sa.String(191), nullable=False),
        sa.Column('amount', sa.Float(), nullable=False),
        sa.Column('currency', sa.String(10), nullable=False, server_default='USD'),
        sa.Column('paddle_transaction_id', sa.String(191), nullable=True),
        sa.Column('status', sa.String(50), nullable=True),
        sa.Column('points_added', sa.Integer(), nullable=True),
        sa.Column('description', sa.Text(), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=True, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['subscription_id'], ['subscriptions.id'], ),
        sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
        sa.PrimaryKeyConstraint('id')
    )


def downgrade() -> None:
    op.drop_table('billing_log')
    op.drop_table('point_transaction')
    op.drop_table('doc_material')
    op.drop_table('media')
    op.drop_table('user')
    op.drop_table('purchased_subscription')
    op.drop_table('subscriptions')
    
    # Drop enum types
    sa.Enum(name='subscriptionplan').drop(op.get_bind())
    sa.Enum(name='accountprovider').drop(op.get_bind())
    sa.Enum(name='useraccounttype').drop(op.get_bind())