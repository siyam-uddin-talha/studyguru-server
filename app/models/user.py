...
class User(Base):
    ...
    account_type = Column(Enum(UserAccountType), default=UserAccountType.USER, nullable=False)
    ...
    # Points system
    current_points = Column(Integer, default=30, nullable=False)  # Free users get 30 points
    total_points_earned = Column(Integer, default=30, nullable=False)
    total_points_used = Column(Integer, default=0, nullable=False)
    
    # Subscription
    purchased_subscription_id = Column(String(191), ForeignKey("purchased_subscription.id"), nullable=True)
    ...
    # Relationships
    purchased_subscription = relationship("PurchasedSubscription", back_populates="users")
